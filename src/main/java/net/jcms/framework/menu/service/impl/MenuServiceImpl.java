...
public class MenuServiceImpl extends BaseServiceImpl<Menu, MenuSearch, MenuMapper> implements MenuService {
	...
	private static List<Menu> userMenuList = null; 
	...
	public List<Menu> selectJson() {
		if(ApplicationHelper.getMenuList() == null) {
			initJson();
		}
		
		return ApplicationHelper.getMenuList();
	}
	...
	public List<Menu> setChildRoleMenuList(List<Menu> menuList, List<String> rlist) {
		...
		for(Menu menu : menuList) {
			...
			menu.setChildList(setChildRoleMenuList(mapper.selectRoleMenuList(menuSearch), rlist));
