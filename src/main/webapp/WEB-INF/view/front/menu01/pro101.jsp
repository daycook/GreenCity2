...
<main>
             	...
             	<div class="clearfix" style="background-color: #e1e1ea; margin-bottom: 10px; padding-right: 10px;">	
					<form id="searchForm" onsubmit="return false" style="float:right; margin-top:25px;">
						...
						<div class="form-group">
							<label for="searchTxt">
								<select id="searchTxt" name="searchTxt" style="width:200px; float: left; margin-right: 10px;" class="form-control" onchange="javascript:fn_site_latlon()">
								</select>
                 ...
                 <div id="map" style="width:100%;height:700px;">
                 	<div id="dataModal1" class="dataModal dataModal1">
						...
						<div style="text-align: center; margin-bottom: 10px;">
							<span id="thisTime" style="font-size: 20px;">실시간현황 (2021년 11월 30일 10시 현재)</span>
						</div>
						<div>
							<table class="subComtTable">
								<tr>
									<td id="nonError" style="background-color: blue; color: white; text-align: center;">정상0개</td>
									<td onclick="javascript:viewErrSiteList(2)" onclick="" id="error2" style="background-color: red; color: white; text-align: center;">수위이상0개</td>
								</tr>
								<tr>
									<td onclick="javascript:viewErrSiteList(1)" id="error1" style="background-color: purple; color: white; text-align: center;">센서미수신0개</td>
									<td onclick="javascript:viewErrSiteList(3)" id="error3" style="background-color: orange; color: white; text-align: center;">통신이상0개</td>
								</tr>
							</table>
						</div>
 					</div>
 					<div id="dataModal2" class="dataModal dataModal2"> <!-- dataModal 넣어야 보임 -->
 						<div style="width: 100%; text-align: right;">
						...
						<div>
							<table class="subComtTable">
								<tr>
									<td id="pumpUseYn" style="background-color: blue; color: white; text-align: center;">펌프 가동 여부 : 비가동중</td>
								</tr>
								<tr>
									<td id="pumpUsage" style="background-color: purple; color: white; text-align: center;">현재 이용량 : 000m3</td>
								</tr>
							</table>
						</div>
 					</div>
 					...
 					<div id="dataModal5" class="dataModal dataModal5">
 						<div style="width: 100%; text-align: right;">
						    <span class="material-icons" style="cursor: pointer;" onclick="javascript:fn_close_modal('dataModal5')">
								<i class="fas fa-times"></i>
							</span>
						</div>
						<div style="text-align: center;">
							<span style="font-size: 20px;" id="seqSiteName5"></span>
						</div>
						<div>
							<div id="chart_div5" style="height: 300px; width: 500px;"></div>
						</div>
 					</div>
 					
 					<div id="dataModal4" class="dataModal4">
						<div style="width: 100%; text-align: right;">
						    <span class="material-icons" style="cursor: pointer;" onclick="javascript:fn_close_modal('dataModal4')">
								<i class="fas fa-times"></i>
							</span>
						</div>
						<div>
							
							<div style="height: 300px; width: 650px; overflow-y: scroll;">
								<table style="width: 100%" class="subComtTable" summary="번호,제목,작성자,첨부,등록일,조회수 항목입니다." id="errSiteTable">
									<thead>
										<tr>
											<th>No</th>
											<th>관측소코드</th>
 ...
 <!-- 로드뷰 모달 -->
 <!-- <div id="roadviewModal" class="mapModal">
      Modal content
      	<div class="mapModal-content">
      		<div id="roadview" style="width: 100%; text-align: right;">
			    <span class="material-icons" style="cursor: pointer;" onclick="javascript:fn_close_modal('roadviewModal')">
					<i class="fas fa-times"></i>
				</span>
			</div>
	  ...
	  $.ajax({
			...
			success : function(result){
				...
				if(maxMinData != null){
			      ...
			      setSituationData(result.situationData);
			   
			      
			      setPumpData(result.pumpData);
			      $(".loading").hide();
			},
			error:function(request,status,error){
		    	alert("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error);
...
function drawChartReal2() {
	  var data = new google.visualization.DataTable();
	  $.ajax({
			url : "<c:url value='/front/statistics/selectMapChartGwDataList.json?gbn=donghwan'/>",
			type : "POST",
			data : {"seqSiteId": seqSiteId},
			dataType : "json",
			success : function(result){
				...
				$("#seqSiteName5").html(seqSiteData.siteName)
    ...
    function setSituationData(data){
    	$("#nonError").html("정상 "+data.nonError+"개");
    	
    	$("#error1").html("센서미수신 "+data.errorType1);
    	$("#error2").html("수위이상 "+data.errorType2);
    	$("#error3").html("통신이상 "+data.errorType3);
    	
    	var date = new Date;
		var html = "실시간현황("+date.getFullYear()+"년 "+(date.getMonth()+1)+"월 "+date.getDate()+"일 "+date.getHours()+"시 현재)"
		$("#thisTime").html(html);
    }
	function setPumpData(data){
		$("#pumpUseYn").html("펌프 가동 여부 : "+data.pumpUseYn);
		$("#pumpUsage").html("현재 이용량 : "+data.pumpUsage+"m3");
	}
	
	//에러관측소 리스트
	function viewErrSiteList(errType){
		$(".loading").show();
		
		if(errType == 2){
			var html = "<tr style='text-align:center;'><td colspan='3'>데이터가 존재하지 않습니다.</td></tr>";
			$("#errSiteTable tbody").empty();
			$("#errSiteTable tbody").append(html);
			$('#dataModal4').show();
			$(".loading").hide();
		}
		$.ajax({
			url : "<c:url value='/front/siteInfo/selectErrSiteList.json'/>",
			type : "POST",
			data : {"errType": errType},
			dataType : "json",
			success : function(result){
				var list = result.selectErrSiteList;
				var leng = list.length;
				var html = "<tr style='text-align:center;'><td colspan='3'>데이터가 존재하지 않습니다.</td></tr>";
				
				for(var i=0; i<leng; i++){
					var data = list[i]
					html = "<tr onclick='javascript:fn_go_siteData();' style='text-align:center; cursor:pointer;'>";
					html += "<td>"+(i+1)+"</td>";
					html += "<td>"+data.siteRpmsno+"</td>";
